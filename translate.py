...
def translate_via_claude_opinion(title, summary):
    ...
    client = get_client()
    prompt = (
        "Your task is to draft a short Telegram post in Russian summarizing "
        "a piece of crypto news, followed by a brief first-person commentary "
        "paragraph (the source material below may already be in Russian, or "
        "in English). Never mention or refer to 'the article', 'the source', "
        "or where the information came from. Keep crypto terms (Bitcoin, "
